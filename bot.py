Чтобы вопросы в игре «Правда или Ложь» были разнообразными, интересными и не повторялись слишком часто, я значительно расширил список (добавил 35 уникальных фактов со всего мира) и добавил программную защиту: теперь бот запоминает, какие вопросы пользователь уже отгадывал в текущей сессии, и не показывает их повторно, пока не закончатся все варианты.
Вот обновленный и полностью готовый код файла bot.py:
import os
import random
import sqlite3
import threading
